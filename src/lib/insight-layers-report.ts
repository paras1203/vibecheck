...
function layerBlockHtml(
  ...
  if (isPaid) {
    const rows = Object.entries(subscores)
      .map(
        ([k, t]) =>
          `<tr><td>${esc(formatSubKey(k))}</td><td class="report-figure">${t.current}</td><td class="report-figure">${t.proposed}</td><td>${esc(t.impact)}</td></tr>`
      )
      .join("");
  ...
  return `<div class="insight-card">
    <div class="insight-card__head">
      ...
      <span class="insight-priority">${esc(priority)} priority</span>
    </div>
    <p class="report-prose" style="font-size:0.9375rem;margin:0;">${esc(layerSummary)}</p>
    <div class="insight-compare report-nojustify">
      <span>Current <span class="report-figure">${composite.current}</span></span>
      <span>→</span>
      <span>Proposed <span class="report-figure">${composite.proposed}</span></span>
    </div>
...
export function buildInsightLayersHtml(
  ...
  const scenarioRow = (label: string, amount: number, rate: number) =>
    `<div class="scenario-row">
      <div class="scenario-row__label">${esc(label)}</div>
      <div class="scenario-row__amt">$${Math.round(amount).toLocaleString()}</div>
  ...
  const revenueCard = `<div class="insight-card">
    <div class="insight-card__head">
      <span class="insight-card__name">Revenue leak estimate</span>
    </div>
    <div style="display:flex;flex-wrap:wrap;gap:10px;margin-bottom:12px;">
      ${scenarioRow("Low", low, sc.low.conversionUpliftRate)}
      ${scenarioRow("Base", base, sc.base.conversionUpliftRate)}
      ${scenarioRow("High", high, sc.high.conversionUpliftRate)}
    </div>
  ...
  return `<div class="section insight-layers report-major">
    ...
    ${revenueCard}
    ${layerBlockHtml(esc, "First impression score", fi, isPaid)}
    ${layerBlockHtml(esc, "Trust gap index", tg, isPaid)}
    ${layerBlockHtml(esc, "Messaging clarity score", mc, isPaid)}
  </div>`;
}
