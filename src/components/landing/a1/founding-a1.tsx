...
export function FoundingA1({ user, loading }: FoundingA1Props) {
  ...
  return (
    <section id="founding" className="border-t border-border bg-background px-4 py-24 md:px-8">
      <div className="container mx-auto max-w-3xl min-w-0">
        <div className="rounded-xl border border-border bg-card p-8 shadow-surface-xs ring-1 ring-primary/15 md:p-10">
          <p className="text-xs font-semibold uppercase tracking-wider text-primary">Founding cohort</p>
          <h2 className="mt-3 text-3xl font-semibold tracking-tight text-foreground md:text-4xl">
            Early access for serious operators
          </h2>
          <p className="mt-4 text-muted-foreground">
            We&apos;re onboarding a small founding cohort—teams who treat conversion work as a priority,
            not a side project. We&apos;re capping it so support stays hands-on while we scale (target:
            first 100 paid accounts).
          </p>
          <ul className="mt-8 list-none space-y-4 text-muted-foreground">
            <li className="flex gap-3">
              <span className="mt-0.5 shrink-0 text-foreground">•</span>
              <span>
                <span className="font-medium text-foreground">Priority handling.</span> Founding members
            ...
            <li className="flex gap-3">
              <span className="mt-0.5 shrink-0 text-foreground">•</span>
              <span>
                <span className="font-medium text-foreground">Tighter iteration.</span> Your feedback
                lands with the people building the product, so the loop from signal to shipped improvement
                stays short.
              </span>
            </li>
            <li className="flex gap-3">
              <span className="mt-0.5 shrink-0 text-foreground">•</span>
              <span>
                <span className="font-medium text-foreground">A real voice in what comes next.</span> We
                read every note; you help shape priorities—we don&apos;t promise a custom roadmap, but we
                do listen.
              </span>
            </li>
          </ul>
          <p className="mt-8 border-t border-border pt-8 text-sm text-muted-foreground">
            If you see strong results, we may ask to feature your story—a short quote or case study—with
            your approval. It&apos;s optional, and it&apos;s a way to get your brand in front of others who
          ...
          <div className="mt-10 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
            ...
            {secondary.useLink ? (
              ...
              <Button variant="outline" asChild className="w-full rounded-xl sm:w-auto">
                <a
                  href={secondary.href}
                  {...(secondary.newTab ? { target: "_blank", rel: "noopener noreferrer" } : {})}
                >
                  {secondary.label}
                </a>
              </Button>
