        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {CARDS.map((c) => (
            <div
              key={c.name}
              className={cn(
                "group rounded-xl border border-[var(--lv-minimal-border)] bg-[var(--lv-minimal-surface-1)] p-6 shadow-surface-xs transition-all hover:-translate-y-1 hover:border-[var(--lv-minimal-accent)]/35 hover:shadow-surface-sm",
              )}
            >
              <IconFrame
                size="lg"
                className="mb-4 border-[var(--lv-minimal-border)] bg-[var(--lv-minimal-bg)] text-[var(--lv-minimal-accent)] transition-transform group-hover:scale-105"
              >
                <c.icon className="size-5 stroke-[1.5]" />
              </IconFrame>
              <h3 className="text-lg font-semibold text-[var(--lv-minimal-text)]">{c.name}</h3>
